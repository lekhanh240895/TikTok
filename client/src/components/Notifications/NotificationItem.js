    ...
    switch (type) {
      ...
      case "mention":
        if (notif.subType === "mention-comment")
          return <span>mentioned you in a comment: {notif.comment.text}</span>;
  ...
  return (
    <li>
      <Comp {...props} className="notification-item">
        <Avatar src={notif.sender.avatar} width="4.8rem" height="4.8rem" />
        ...
        {notif.type === "follow" ? (
          isFollowed ? (
            <Button
              ...
              leftIcon={<TwoWayArrow width="1.4rem" height="1.4rem" />}
            >
              Friends
            </Button>
          ) : (
