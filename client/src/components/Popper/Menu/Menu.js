          ...
          onClick={async () => {
            if (isParent) {
              ...
              if (item.title === "Delete") {
                if (item.videoID) {
                  dispatch(appSlice.actions.setIsDeleteModalShow(true));
                  dispatch(appSlice.actions.setSelectedVideoID(item.videoID));
                }
                if (item.commentID) {
  ...
  return (
    <div>
      <HeadlessTippy
        ...
        offset={offset}
        render={(attrs) => (
          <div className={cx("wrapper")} tabIndex="-1" {...attrs} {...props}>
            <PopperWrapper {...props}>
              {history.length > 1 && (
              ...
              >
                ...
                {moreArrow & !isMore ? (
                  ...
                  >
                    <DownArrow width="2.4rem" height="2.4rem" />
                  </div>
                ) : null}
              </div>
            </PopperWrapper>
