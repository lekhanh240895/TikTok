...
export default function Header({ video, currentUser, socket, comments }) {
  ...
  const handleUnFollow = () => {
    ...
    const updatedUser = {
      ...currentUser,
      followings: currentUser.followings.filter((id) => id !== video.user._id),
    };
    dispatch(authSlice.actions.setCurrentUser(updatedUser));
      ...
      <div className="user-info">
        ...
        <Link to={`/@${video.user.username}`} className="info">
          <h4>
            {video.user.username}
            {video.user.tick && (
              <CheckedIcon className="check" width="1.4rem" height="1.4rem" />
            )}
          </h4>
          <p>
        ...
        {isUser ? (
          <Menu items={OPTION_MENU} placement="bottom-end" paddingMenu>
            <span className="option-icon-wrapper icon-wrapper">
              <OptionIcon width="2.4rem" height="2.4rem" />
            </span>
          </Menu>
        ) : !isFollowed ? (
          <Button primary className="follow-btn" onClick={handleFollow}>
            Follow
          </Button>
        ) : (
          <Button secondary className="follow-btn" onClick={handleUnFollow}>
            Following
          </Button>
        )}
      </div>
      ...
      <div className="video-info">
        <div className="like-comment">
          <span className="likes" onClick={handleLike}>
            <span
              className={
                isLiked ? "icon-wrapper-circle liked" : "icon-wrapper-circle"
              }
            >
              <SolidHeartIcon width="2rem" height="2rem" />
            </span>
            <span className="likes-number">{configNumber(likes)}</span>
          </span>
