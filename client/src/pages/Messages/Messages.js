...
export default function Messages() {
  ...
  const handleDeleteConversation = async (conversationID) => {
    ...
    if (newConversations.length > 0) {
      dispatch(
        appSlice.actions.setSelectedConversationID(newConversations[0]._id)
      );
    }
  };
  ...
  return (
    <Wrapper>
      ...
      <div
        className={
          selectedConversationID ? "container mobile-has-selectID" : "container"
        }
      >
        ...
        <div className="back-icon icon-wrapper" onClick={() => navigate(-1)}>
          <LeftArrowIcon2 />
        </div>
      </div>
    </Wrapper>
